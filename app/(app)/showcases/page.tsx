...
export default async function ShowcasesPage() {
  ...
  const rows = await db.select().from(showcases).orderBy(desc(showcases.createdAt));
  ...
  return (
    <>
      <PageHeader
        eyebrow="Sales"
        title="Sample work"
        sub="Build a zoomable showcase of your finishing & furniture and share it with a client via one private link."
        actions={
          <Link href="/showcases/new" className="rounded-full bg-ink px-4 py-2 text-sm font-semibold text-cream hover:bg-ink/90">+ New showcase</Link>
        }
      />
        ...
        <Card className="overflow-hidden">
          {rows.length === 0 ? (
            <div className="p-10 text-center text-sm text-sub">
              No showcases yet. <Link href="/showcases/new" className="font-semibold text-olive">Create your first one →</Link>
            </div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-line text-left text-xs font-bold uppercase tracking-wider text-muted">
                  <th className="px-5 py-3">Showcase</th>
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-line/60 last:border-0 hover:bg-sand/40">
                    <td className="px-5 py-3">
                      <Link href={`/showcases/${r.id}`} className="font-semibold text-ink hover:text-olive">{r.title}</Link>
